 ...
 */ (function($, window1, document1, undefined) {
    ...
    $.extend(FullView.prototype, {
        ...
        buildCache: function() {
            ...
            if (this.options.navbar !== undefined && typeof this.options.navbar === 'string') {
                if ($(this.options.navbar).length) this.$navbar = $(this.options.navbar);
            }
        },
        utilites: function() {
            this.createDots = function createDots() {
                var $dots = $("#fv-dots");
                if ($dots.length) $dots.remove();
                ...
                if (this.options.dotsTooltips === true) {
                    ...
                    this.$views.each(function(i) {
                        ...
                        if (tooltipTitle) div.find('ul').append('<li class="fv-tooltip"><a data-scroll="' + i + '" href="#" class=""><span></span></a><span class="fv-tooltiptext">' + tooltipTitle + '</span></li>');
                        else div.find('ul').append('<li class="fv-tooltip"><a data-scroll="' + i + '" href="#" class=""><span></span></a></li>');
                    });
                } else this.$views.each(function(i) {
            ...
            this.changeActiveStatus = function changeActiveStatus($view) {
                ...
                if (this.options.dots) this.$dotsElement.removeClass('active').eq($view).addClass('active');
                if (this.$anchors.length) this.$anchors.removeClass('active').filter('[data-scroll="' + $view + '"]').addClass('active');
            };
            this.scrollTo = function scrollTo($view, dir) {
                ...
                if (this.offsets[$view] !== undefined && typeof $view === 'number') {
                    ...
                    if (dir === undefined) {
                        if (this.previousView > this.currentView) dir = "up";
                        else if (this.previousView < this.currentView) dir = "down";
                    }
                    // Event OnScrollStart
                    ...
                    this.$htmlBody.stop(true).animate({
                        ...
                        if (plugin.isScrolling === true) setTimeout(function() {
                ...
                } else console.warn("The View You Want To Scroll To Does not Exist!");
            };
            ...
            this.scrollUp = function scrollUp() {
                ...
                if (this.currentView > 0) {
                    ...
                    this.currentView--;
                    this.scrollTo(this.currentView, dir);
                } else if (this.currentView === 0) this.isScrolling = false;
                return this;
            };
        },
        settingUp: function() {
            ...
            this.currentView = 0;
            this.isScrolling = false;
            document1.body.scrollTop = 0;
            document1.documentElement.scrollTop = 0;
            if (typeof this.options.speed !== 'number') {
                this.options.speed = 500;
                console.warn("Speed Should be of Type Number");
            } else if (this.options.speed <= 300) {
            ...
            if (this.$navbar !== null) {
                ...
                var seletedAnchor = this.offsets.filter(function(obj) {
                    if (obj.anchor !== undefined) {
                        ...
                        if ($menuitem.length) $menuitem.attr("data-scroll", obj.position);
                        return true;
                    }
                    return false; // skip
                }).map(function(obj) {
            ...
            // Creating Dots
            if (this.options.dots) this.$dotsElement = this.createDots();
            ...
            if ($actview.length) {
                ...
                this.previousView = this.currentView;
                document1.body.scrollTop = offset;
                document1.documentElement.scrollTop = offset;
            }
            // If AutoPlay
            if (this.options.dots) this.$dotsElement = this.createDots();
        ...
        bindEvents: function() {
            ...
            // On Dot Click
            plugin.$dotsElement !== null && plugin.$dotsElement.on("click." + plugin._name, function(e) {
                e.preventDefault();
                if (!$(':animated').length) {
            ...
            // On nav anchor click
            plugin.$anchors !== undefined && plugin.$anchors.length > 0 && plugin.$anchors.on("click." + plugin._name, function(e) {
                e.preventDefault();
                if (!$(':animated').length) {
                    plugin.previousView = plugin.currentView;
                    plugin.currentView = parseInt($(this).attr("data-scroll"));
            ...
            // On MouseScroll
            plugin.options.mouseScrolling && plugin.$window.on("DOMMouseScroll mousewheel." + plugin._name, function(event) {
                var e = event || window1.event, target = e.target || e.srcElement;
                if (target.tagName.toUpperCase() == 'INPUT') return;
                plugin.scrollByWheel(e);
            });
            // On Keyboard Press
            plugin.options.keyboardScrolling && plugin.$document.on("keydown." + plugin._name, function(event) {
                var e = event || window1.event, target = e.target || e.srcElement;
                if (target.tagName.toUpperCase() == 'INPUT') return;
                // Check if Already scrolling
            ...
            plugin.options.touchScrolling && plugin.$views.on("touchend." + plugin._name, function(event) {
                ...
                if (plugin.lastY > currentY + 25) plugin.scrollDown();
                else if (plugin.lastY < currentY - 25) plugin.scrollUp();
            });
        },
        ...
        callback: function(eventName, addtional) {
            ...
            if (typeof onScrollEnd === 'function' && eventName === "onScrollEnd") // Current, Previous, Direction
            onScrollEnd(this.$views.eq(this.currentView), this.$views.eq(this.previousView), addtional.direction);
            if (typeof onScrollStart === 'function' && eventName === "onScrollStart") // Current, Destination, Direction
    ...
    $.fn.fullView = function(options) {
        if (options === undefined || typeof options === 'object') return this.each(function() {
            if (!$.data(this, fullView)) $.data(this, fullView, new FullView(this, options));
        });
        return this;
    };
...
//# sourceMappingURL=TimeTogether.35ffbc9c.js.map
